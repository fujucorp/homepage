...
namespace CancelProcess
{
    /// <summary>
    /// 銷帳處理 (SCD：銷帳處理、SCD2：銷帳處理(補))
    /// </summary>
    class Program
    {
        #region [OLD]
        ...
        ///// <summary>
        ///// 參數：service_id=所屬作業類別代碼(SCD、SCD2) account_date=要處理的入帳日(TODAY, YESTERDAY 或 yyyyMMdd) days=從 account_date 開始（含）往前處理的天數限 SCD2 有效
        ///// </summary>
        ...
        //    JobCubeCheckMode jobCheckMode = JobCubeCheckMode.Skip;  //因為被處理的資料會註記，所以不用檢查
        ...
        //                if (args == null || args.Length == 0)
        //                {
        //                    log.AppendLine("參數語法：service_id=所屬作業類別代碼(SCD、SCD2) account_date=要處理的入帳日(TODAY, YESTERDAY 或 yyyyMMdd)");
        //                }
        //            }
        //        }
        //        #endregion
        ...
        //            string account_date = Common.GetTWDate7(accountDate.Value);   //轉成民國年7碼
        //            if (!cancel_helper.CancelByAccountDate(account_date, ref jobLog))
        //            {
        //                exitCode = -3;
        //                exitMsg = String.Format("銷帳處理發生錯誤，錯誤訊息：{0}", cancel_helper.err_msg);
        ...
        //            else
        //            {
        //                if (String.IsNullOrWhiteSpace(cancel_helper.err_msg))
        //                {
        //                    exitMsg = "銷帳處理完成";
        //                    jobLog.AppendFormat("[{0:yyyy/MM/dd HH:mm:ss}] {1}", DateTime.Now, exitMsg).AppendLine();
        //                    log.AppendFormat("[{0:yyyy/MM/dd HH:mm:ss}] {1}", DateTime.Now, exitMsg).AppendLine();
        ...
        //                else
        //                {
        //                    exitMsg = String.Format("銷帳處理完成，訊息：{0}", cancel_helper.err_msg);
        //                    jobLog.AppendFormat("[{0:yyyy/MM/dd HH:mm:ss}] {1}", DateTime.Now, exitMsg).AppendLine();
        //                    log.AppendFormat("[{0:yyyy/MM/dd HH:mm:ss}] {1}", DateTime.Now, exitMsg).AppendLine();
        //                }
        //            }
        ...
        /// <remarks>
        /// 執行參數說明
        /// service_id：指定排程作業類別代碼，可指定銷帳處理（SCD）、銷帳處理(補)（SCD2）。必要參數
        /// account_date：指定處理的資料日，可指定今天（TODAY）、昨天（YESTERDAY）、特定日期（西元年 yyyyMMdd）（ATM、網路銀行取代收日，非ATM、網路銀行取入帳日）。必要參數
        /// days：指定 SCD2 的回溯天數 (從 account_date 開始往回處理的天數)，可指定 0 ~ 7，限 SCD2 有效。非必要參數
        /// </remarks>
        static void Main(string[] args)
        {
            #region 變數初始化
            ...
            string jobStamp = null;
            JobCubeCheckMode jobCheckMode = JobCubeCheckMode.Skip;  //因為被處理的資料會註記，所以不用檢查
            string jobTypeId = null;        //作業類別代碼 (SCD、SCD2)
            string jobTypeName = null;      //作業類別名稱
            StringBuilder jobLog = new StringBuilder();
            ...
            try
            {
                ...
                #region 處理參數
                DateTime? accountDate = null;  //資料日期
                ...
                if (exitCode == 0)
                {
                                ...
                                switch (key)
                                {
                                    case "service_id":
                                        #region jobTypeId (作業類別代碼)
                                        {
                                            jobTypeId = value;
                                            jobTypeName = JobCubeTypeCodeTexts.GetText(jobTypeId);
                                            if ((jobTypeId != JobCubeTypeCodeTexts.SCD && jobTypeId != JobCubeTypeCodeTexts.SCD2) || String.IsNullOrEmpty(jobTypeName))
                                            {
                                                errmsg = "service_id 參數值不是正確的作業類別代碼";
                                            }
                                        }
                    ...
                    if (String.IsNullOrEmpty(errmsg))
                    {
                        if (jobTypeId == JobCubeTypeCodeTexts.SCD2)
                        {
                            if (!days.HasValue)
                            {
                                days = 0;
                        ...
                        else
                        {
                            if (days.HasValue)
                            {
                                errmsg = "service_id 不是 " + JobCubeTypeCodeTexts.SCD2 + " 時 days 參數不可使用";
                            }
                        }
                    }
                    #endregion
                    ...
                    if (!String.IsNullOrEmpty(errmsg))
                    {
                        ...
                        fileLog
                            ...
                            .AppendLineLog("參數語法：service_id=作業類別代碼(SCD、SCD2) account_date=資料日期(TODAY、YESTERDAY、yyyyMMdd) [days=回溯天數(0~7 限SCD2使用)]");
                    }
                }
                #endregion
                ...
                if (exitCode == 0)
                {
                    ...
                    if (jobTypeId == JobCubeTypeCodeTexts.SCD)
                    {
                        #region SCD 銷帳處理
                        string tw7AccountDate = Common.GetTWDate7(accountDate.Value);   //轉成民國年7碼
                        if (cancelHelper.CancelByAccountDate(tw7AccountDate, ref jobLog))
                        {
                            ...
                            {
                                exitMsg = String.Format("銷帳處理完成，訊息：{0}", cancelHelper.err_msg);
                            }
                        }
                        else
                        {
                            exitCode = -3;  //銷帳處理失敗回傳代碼
                            exitMsg = String.Format("銷帳處理發生錯誤，錯誤訊息：{0}", cancelHelper.err_msg);
                        }
                        jobLog.AppendFormat("[{0:yyyy/MM/dd HH:mm:ss}] {1}", DateTime.Now, exitMsg).AppendLine();
                    ...
                    else
                    {
                        ...
                        {
                            string tw7AccountDate = Common.GetTWDate7(checkDate);   //轉成民國年7碼
                            jobLog.AppendFormat("開始處理 {0} 的資料", tw7AccountDate).AppendLine();
                            fileLog.AppendFormatLog("開始處理 {0} 的資料", tw7AccountDate).AppendLineLog();
                            if (cancelHelper.CancelByAccountDate(tw7AccountDate, ref jobLog))
                            {
                                if (String.IsNullOrWhiteSpace(cancelHelper.err_msg))
                                {
