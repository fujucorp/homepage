...
{
    public partial class D1400002M1 : BasePage
    {
        #region Property
        /// <summary>
        /// 編輯模式參數
        ...
        /// 儲存對照表Xlsmdb
        /// </summary>
        private MappingrtXlsmdbEntity EditMappingrtXlsmdbEntity
        {
            get
            {
                return ViewState["EditMappingrtXlsmdbEntity"] as MappingrtXlsmdbEntity;
            }
            set
            {
                ViewState["EditMappingrtXlsmdbEntity"] = value == null ? null : value;
            }
        }
        ...
        /// 儲存對照表Txt
        /// </summary>
        private MappingrtTxtEntity EditMappingrtTxtEntity
        {
            get
            {
                return ViewState["EditMappingrtTxtEntity"] as MappingrtTxtEntity;
            }
            set
            {
                ViewState["EditMappingrtTxtEntity"] = value == null ? null : value;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                MappingrtXlsmdbEntity XlsData = null;
                MappingrtTxtEntity TxtData = null;
                ...
                switch (this.Action)
                {
                    case "M":   //修改
                        #region 取得修改的資料
                        {
                            ...
                            {
                                Expression where = new Expression(MappingrtXlsmdbEntity.Field.ReceiveType, this.EditReceiveType);
                                where.And(MappingrtXlsmdbEntity.Field.MappingId, this.EditMappingId);
                                result = DataProxy.Current.SelectFirst<MappingrtXlsmdbEntity>(this, where, null, out XlsData);

                            }
                            else
                            {
                                Expression where = new Expression(MappingrtTxtEntity.Field.ReceiveType, this.EditReceiveType);
                                where.And(MappingrtTxtEntity.Field.MappingId, this.EditMappingId);
                                result = DataProxy.Current.SelectFirst<MappingrtTxtEntity>(this, where, null, out TxtData);
                            }
                            if (result.IsSuccess)
                            {
        ...
        private void BindXlsData(MappingrtXlsmdbEntity entity)
        {
            ...
            else
            {
                ...
                this.EditMappingrtXlsmdbEntity = entity;
            }
            ...
            switch (this.Action)
            {
                ...
                case "M":
                    ...
                    for (int i = 1; i < 30; i++)
                    {
                        string strFieldName = "Rt_" + i.ToString("00");
                    ...
                    if (iRtCount > 0)
                    {
                        CheckBox ck = (CheckBox)cph.FindControl("chkRt");
                        if (ck != null)
                        {
                            ck.Checked = true;
                        }
                        tbxRtCount.Text = iRtCount.ToString();
                    }
                    #endregion
        ...
        /// </summary>
        /// <param name="option">主要條件選項資料</param>
        private void BindTxtData(MappingrtTxtEntity entity)
        {
            if (entity == null && Action == "M")
            {
                this.lbtnNext.Visible = false;
            ...
            {
                this.lbtnNext.Visible = true;
                this.EditMappingrtTxtEntity = entity;
            }
            ...
            switch (this.Action)
            {
                ...
                case "M":
                    ...
                    for (int i = 1; i < 30; i++)
                    {
                        string strFieldName = "Rt_" + i.ToString("00") + "_S";
                        object ob = entity.GetValue(strFieldName, out rt);
                        if (ob != null)
                        {
                            if (!String.IsNullOrEmpty(ob.ToString()))
        ...
        private bool InitialUI()
        {
            ...
            foreach (Control control in cph.Controls)
            {
                if (control is CheckBox)
                {
                    ...
                    if (ck.ID != "chkStu_Id") ck.Checked = false;
                }
            }
            tbxMappingName.Text = "";
            tbxRtCount.Text = "";
                ...
                foreach (Control control in cDiv.Controls)
                {
                    if (control is CheckBox)
                    {
                        ...
                        if (ck.Checked)
                        {
                            ...
                            {
                                case "chkRt":
                                    iCount = Convert.ToInt16(((TextBox)cDiv.FindControl("tbxRtCount")).Text);
                                    for (int i = 1; i <= iCount; i++)
                                    {
                                        strID = "Rt_" + i.ToString("00");
        ...
        protected void lbtnNext_Click(object sender, EventArgs e)
        {
            ...
            //KeyValueList<string> lists = GetCheckedItems();
            ...
            if (ddlFileType.SelectedValue == "xls")  //xls
            {
                ...
                //組 Html
                initStep2B(this.EditMappingrtXlsmdbEntity);
            }
            else
            {
                divStep1.Visible = false;
                ...
                //組 Html
                initStep2A(this.EditMappingrtTxtEntity);
            }
        }

        protected void initStep2B(MappingrtXlsmdbEntity entity)
        {
        ...
        protected void initStep2A(MappingrtTxtEntity entity)
        {
            List<MappingItem> items = this.EditMappingItems;
        ...
        public string CheckDataForInputUI()
        {
            ...
            if (chkRt.Checked)
            {
                if (String.IsNullOrEmpty(tbxRtCount.Text))
                {
                    return this.GetLocalized("請輸入") + this.GetLocalized("退費金額項目");
                }
            }
        ...
        protected void lbtnOK2A_Click(object sender, EventArgs e)
        {
            ...
            if (Action == "A")
            {
                mappingId = GetNewMappingID(MappingrtTxtEntity.TABLE_NAME);
            }
            else
            {
            ...
            itemNames.Add("tableName", MappingrtTxtEntity.TABLE_NAME);
        ...
        protected void lbtnOK2B_Click(object sender, EventArgs e)
        {
            ...
            if (Action == "A")
            {
                mappingId = GetNewMappingID(MappingrtXlsmdbEntity.TABLE_NAME);
            }
            else
            {
                mappingId = this.EditMappingId;
            ...
            itemNames.Add("tableName", MappingrtXlsmdbEntity.TABLE_NAME);
            itemNames.Add("receiveType", receiveType);
            itemNames.Add("mappingId", mappingId);
        ...
        {
            if (tableName == MappingrtTxtEntity.TABLE_NAME)
            {
                MappingrtTxtEntity entity = null;
                Expression where = new Expression(MappingrtTxtEntity.Field.ReceiveType, this.EditReceiveType);
                KeyValueList<OrderByEnum> orderbys = new KeyValueList<OrderByEnum>();
                orderbys.Add(MappingrtTxtEntity.Field.MappingId, OrderByEnum.Desc);

                XmlResult result = DataProxy.Current.SelectFirst<MappingrtTxtEntity>(this, where, orderbys, out entity);
                if (result.IsSuccess)
                {
                    if (entity == null)
                    {
                    ...
                    {
                        int iCount = Convert.ToInt16(entity.MappingId);
            ...
            {
                MappingrtXlsmdbEntity entity = null;
                Expression where = new Expression(MappingrtXlsmdbEntity.Field.ReceiveType, this.EditReceiveType);
                KeyValueList<OrderByEnum> orderbys = new KeyValueList<OrderByEnum>();
                orderbys.Add(MappingrtXlsmdbEntity.Field.MappingId, OrderByEnum.Desc);

                XmlResult result = DataProxy.Current.SelectFirst<MappingrtXlsmdbEntity>(this, where, orderbys, out entity);
                if (result.IsSuccess)
                {
                    if (entity == null)
                    {
