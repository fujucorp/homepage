...
{
    public partial class D1300010 : PagingBasePage
    {
        ...
        protected override XmlResult GetWhereAndOrderBys(out Expression where, out KeyValueList<OrderByEnum> orderbys)
        {
            ...
            where.And(ReturnStandardEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(ReturnStandardEntity.Field.YearId, this.QueryYearID);
            where.And(ReturnStandardEntity.Field.TermId, this.QueryTermID);
            where.And(ReturnStandardEntity.Field.DepId, this.QueryDepID);
            where.And(ReturnStandardEntity.Field.ReceiveId, this.QueryReceiveID);


            orderbys = new KeyValueList<OrderByEnum>(1);
            orderbys.Add(ReturnStandardEntity.Field.ReturnId, OrderByEnum.Asc);
        ...
        protected override XmlResult CallQueryDataAndBind(PagingInfo pagingInfo)
        {
            ...
            XmlResult xmlResult = base.QueryDataAndBind<ReturnStandardEntity>(pagingInfo, ucPagings, this.gvResult);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        QueryString.Add("ReturnId", returnId);
                        Session["QueryString"] = QueryString;

                        #region [MDY:20210521] 原碼修正
                        Server.Transfer(WebHelper.GenRNUrl("D1300010M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        int count = 0;
                        XmlResult result = DataProxy.Current.Delete<ReturnStandardEntity>(this, entity, out count);
                        if (result.IsSuccess)
                        {
                            if (count == 0)
                            {
        ...
        /// 取得減免類別名稱
        /// </summary>
        protected ReturnListEntity[] getAllDatas()
        {
            ReturnListEntity[] datas = null;
            Expression where = new Expression();
            where.And(ReturnListEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(ReturnListEntity.Field.YearId, this.QueryYearID);
            where.And(ReturnListEntity.Field.TermId, this.QueryTermID);
            where.And(ReturnListEntity.Field.DepId, this.QueryDepID);
        ...
        /// <returns></returns>
        public static string GetReturnText(ReturnListEntity[] allDatas, string code, string defaultText)
        {
            if (allDatas != null && code != null && code.Length > 0)
            {
                foreach (ReturnListEntity data in allDatas)
                {
                    if (data.ReturnId == code)
                    {
        ...
        {
            //院別名稱
            ReturnListEntity[] allDataList = getAllDatas();


            ReturnStandardEntity[] datas = this.gvResult.DataSource as ReturnStandardEntity[];
            ...
            {
                ReturnStandardEntity data = datas[row.RowIndex];

                //顯示減免名稱
                row.Cells[1].Text = GetReturnText(allDataList, data.ReturnId, string.Empty);

                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4},{5}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReceiveId, data.ReturnId);
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1300010M.aspx");
        }
    }
}