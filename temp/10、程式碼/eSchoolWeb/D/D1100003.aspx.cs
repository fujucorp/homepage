...
namespace eSchoolWeb.D
{
    ...
    /// 代收費用別代碼
    /// </summary>
    public partial class D1100003 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetAndBindQueryData()
        {
            ...
            XmlResult xmlResult = DataProxy.Current.SelectAll<ReceiveListEntity>(this, where, orderbys, out datas);
            if (!xmlResult.IsSuccess)
            {
        ...
        private Expression GetWhere()
        {
            ...
            {
                where.And(ReceiveListEntity.Field.ReceiveType, receiveType);
            }

            string yearId = this.QueryYearID;
            if (!String.IsNullOrEmpty(yearId))
            {
                where.And(ReceiveListEntity.Field.YearId, yearId);
            }

            string termId = this.QueryTermID;
            if (!String.IsNullOrEmpty(termId))
            {
                where.And(ReceiveListEntity.Field.TermId, termId);
            }

            string DepId = this.QueryDepID;
            if (!String.IsNullOrEmpty(DepId))
            {
                where.And(ReceiveListEntity.Field.DepId, DepId);
            }
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1100003M.aspx");
        }

        protected void gvResult_PreRender(object sender, EventArgs e)
        {
            ReceiveListEntity[] datas = this.gvResult.DataSource as ReceiveListEntity[];
            ...
            {
                ReceiveListEntity data = datas[row.RowIndex];
                //資料參數
