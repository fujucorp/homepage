...
{
    public partial class D1300009 : PagingBasePage
    {
        ...
        protected override XmlResult GetWhereAndOrderBys(out Expression where, out KeyValueList<OrderByEnum> orderbys)
        {
            ...
            where.And(ReduceStandardEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(ReduceStandardEntity.Field.YearId, this.QueryYearID);
            where.And(ReduceStandardEntity.Field.TermId, this.QueryTermID);
            where.And(ReduceStandardEntity.Field.DepId, this.QueryDepID);
            where.And(ReduceStandardEntity.Field.ReceiveId, this.QueryReceiveID);


            orderbys = new KeyValueList<OrderByEnum>(1);
            orderbys.Add(ReduceStandardEntity.Field.ReduceId, OrderByEnum.Asc);
        ...
        protected override XmlResult CallQueryDataAndBind(PagingInfo pagingInfo)
        {
            ...
            XmlResult xmlResult = base.QueryDataAndBind<ReduceStandardEntity>(pagingInfo, ucPagings, this.gvResult);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        QueryString.Add("ReduceId", reduceId);
                        Session["QueryString"] = QueryString;

                        #region [MDY:20210521] 原碼修正
                        Server.Transfer(WebHelper.GenRNUrl("D1300009M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        entity.ReduceId = reduceId;
                        #endregion

                        int count = 0;
                        XmlResult result = DataProxy.Current.Delete<ReduceStandardEntity>(this, entity, out count);
                        if (result.IsSuccess)
                        {
                            if (count == 0)
                            {
        ...
        /// 取得減免類別名稱
        /// </summary>
        protected ReduceListEntity[] getAllDatas()
        {
            ReduceListEntity[] datas = null;
            Expression where = new Expression();
            where.And(ReduceListEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(ReduceListEntity.Field.YearId, this.QueryYearID);
            where.And(ReduceListEntity.Field.TermId, this.QueryTermID);
            where.And(ReduceListEntity.Field.DepId, this.QueryDepID);
        ...
        /// <returns></returns>
        public static string GetReduceText(ReduceListEntity[] allDatas, string code, string defaultText)
        {
            if (allDatas != null && code != null && code.Length > 0)
            {
                foreach (ReduceListEntity data in allDatas)
                {
                    if (data.ReduceId == code)
                    {
                        return data.ReduceName;
                    }
                }
            }
            return defaultText;
        ...
        {
            //院別名稱
            ReduceListEntity[] allDataList = getAllDatas();


            ReduceStandardEntity[] datas = this.gvResult.DataSource as ReduceStandardEntity[];
            ...
            {
                ReduceStandardEntity data = datas[row.RowIndex];

                //顯示減免名稱
                row.Cells[1].Text = GetReduceText(allDataList, data.ReduceId, string.Empty);

                //減免方式: 1 - 依百分比計算 2 - 依金額計算 3 - 金額依次減免
                switch (data.ReduceWay)
                {
                    case "1":
                    ...
                    case "2":
                        ...
                        break;
                    case "3":
                        row.Cells[2].Text = "金額依次減免";
                        break;
                }

                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4},{5}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReceiveId, data.ReduceId);
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1300009M.aspx");
        }
    }
}