...
namespace eSchoolWeb.D
{
    ...
    /// 住宿代碼
    /// </summary>
    public partial class D1100008 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetAndBindQueryData()
        {
            ...
            XmlResult xmlResult = DataProxy.Current.SelectAll<DormListEntity>(this, where, orderbys, out datas);
            if (!xmlResult.IsSuccess)
            {
        ...
        private Expression GetWhere()
        {
            ...
            {
                where.And(DormListEntity.Field.ReceiveType, receiveType);
            }

            string yearId = this.QueryYearID;
            if (!String.IsNullOrEmpty(yearId))
            {
                where.And(DormListEntity.Field.YearId, yearId);
            }

            string termId = this.QueryTermID;
            if (!String.IsNullOrEmpty(termId))
            {
                where.And(DormListEntity.Field.TermId, termId);
            }

            string DepId = this.QueryDepID;
            if (!String.IsNullOrEmpty(DepId))
            {
                where.And(DormListEntity.Field.DepId, DepId);
            }
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1100008M.aspx");
        }

        protected void gvResult_PreRender(object sender, EventArgs e)
        {
            DormListEntity[] datas = this.gvResult.DataSource as DormListEntity[];
            ...
            {
                DormListEntity data = datas[row.RowIndex];
                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.DormId);
