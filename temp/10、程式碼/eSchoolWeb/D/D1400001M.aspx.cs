...
namespace eSchoolWeb.D
{
    ...
    public partial class D1400001M : BasePage
    {
        ...
        private bool CheckXlsMappingItems(List<MappingItem> mappingItems)
        {
            ...
            #region [MDY:202203XX] 2022擴充案 備註項目對照欄位 & 英文標題欄位
            ...
            foreach (MappingItem item in mappingItems)
            {
                if (item.IsChecked && item.MaxItemCount == 0)
                {
                    ...
                    #region [MDY:202203XX] 2022擴充案 增加收入科目英文名稱、備註項目英文標題
        ...
        private List<MappingItem> GetMappingItems(MappingreXlsmdbEntity data)
        {
            ...
            #region 單一項目的欄位
            {
                string[] fields = new string[] {
                    ...
                    #region [MDY:202203XX] 2022擴充案 增加 學籍資料英文名稱對照欄位
                    ...
                    #region [MDY:202203XX] 2022擴充案 增加 減免、就貸、住宿英文名稱對照欄位
                    ...
                    #region [MDY:202203XX] 2022擴充案 增加 身分註記英文名稱對照欄位
            ...
            #region 收入科目金額欄位
            #region [MDY:202203XX] 2022擴充案 增加收入科目英文名稱欄位
            {
                string[] fields = new string[] {
                    MappingreXlsmdbEntity.Field.Receive1, MappingreXlsmdbEntity.Field.Receive2, MappingreXlsmdbEntity.Field.Receive3,
                    MappingreXlsmdbEntity.Field.Receive4, MappingreXlsmdbEntity.Field.Receive5, MappingreXlsmdbEntity.Field.Receive6,
...
<td>{name}</td>
<td>
    <input type=""text"" name=""tbx{key}"" id=""tbx{key}"" value=""{value}"" maxlength=""{item.XlsNameSize}"" />
    <span style=""padding: 0 0 0 5px;"">英文名稱：<input type=""text"" name=""tbx{engKey}"" id=""tbx{engKey}"" value=""{HttpUtility.HtmlEncode(item.EngFieldValue)}"" maxlength=""{item.XlsNameSize}"" /></span>
</td>
</tr>";
            }
            else
            {
                return $@"<tr>
<td>{name}</td>
<td><input type=""text"" name=""tbx{key}"" id=""tbx{key}"" value=""{value}"" maxlength=""{item.XlsNameSize}"" /></td>
</tr>";
            }
        }
...
<td>
    <input type=""text"" name=""tbx{chtKey}"" id=""tbx{chtKey}"" value=""{HttpUtility.HtmlEncode(chtItem.XlsName)}"" maxlength=""{chtItem.XlsNameSize}"" />
    <span style=""padding: 0 0 0 5px;"">英文欄位：<input type=""text"" name=""tbx{engKey}"" id=""tbx{engKey}"" value=""{HttpUtility.HtmlEncode(engItem.XlsName)}"" maxlength=""{engItem.XlsNameSize}"" /></span>
</td>
</tr>";
        }
