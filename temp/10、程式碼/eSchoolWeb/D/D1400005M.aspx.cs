...
{
    public partial class D1400005M1 : BasePage
    {
        #region Property
        /// <summary>
        /// 編輯模式參數
        ...
        /// 儲存對照表Xlsmdb
        /// </summary>
        private MappingrrXlsmdbEntity EditMappingrrXlsmdbEntity
        {
            get
            {
                return ViewState["EditMappingrrXlsmdbEntity"] as MappingrrXlsmdbEntity;
            }
            set
            {
                ViewState["EditMappingrrXlsmdbEntity"] = value == null ? null : value;
            }
        }
        ...
        /// 儲存對照表Txt
        /// </summary>
        private MappingrrTxtEntity EditMappingrrTxtEntity
        {
            get
            {
                return ViewState["EditMappingrrTxtEntity"] as MappingrrTxtEntity;
            }
            set
            {
                ViewState["EditMappingrrTxtEntity"] = value == null ? null : value;
            }
        }
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!this.IsPostBack)
            {
                ...
                MappingrrXlsmdbEntity XlsData = null;
                MappingrrTxtEntity TxtData = null;
                ...
                switch (this.Action)
                {
                    case "M":   //修改
                        #region 取得修改的資料
                        {
                            ...
                            {
                                Expression where = new Expression(MappingrrXlsmdbEntity.Field.ReceiveType, this.EditReceiveType);
                                where.And(MappingrrXlsmdbEntity.Field.MappingId, this.EditMappingId);
                                result = DataProxy.Current.SelectFirst<MappingrrXlsmdbEntity>(this, where, null, out XlsData);

                            }
                            else
                            {
                                Expression where = new Expression(MappingrrTxtEntity.Field.ReceiveType, this.EditReceiveType);
                                where.And(MappingrrTxtEntity.Field.MappingId, this.EditMappingId);
                                result = DataProxy.Current.SelectFirst<MappingrrTxtEntity>(this, where, null, out TxtData);
                            }
                            if (result.IsSuccess)
                            {
        ...
        private void BindXlsData(MappingrrXlsmdbEntity entity)
        {
            ...
            else
            {
                ...
                this.EditMappingrrXlsmdbEntity = entity;
            }
            ...
            switch (this.Action)
            {
                ...
                case "M":
                    ...
                    for (int i = 1; i < 30; i++)
                    {
                        string strFieldName = "RR_" + i.ToString();
                    ...
                    if (iCount > 0)
                    {
                        CheckBox ck = (CheckBox)cph.FindControl("chkRR");
                        if (ck != null)
                        {
                            ck.Checked = true;
                        }
                        tbxRRCount.Text = iCount.ToString();
                    }
                    #endregion
        ...
        /// </summary>
        /// <param name="option">主要條件選項資料</param>
        private void BindTxtData(MappingrrTxtEntity entity)
        {
            if (entity == null && Action == "M")
            {
                this.lbtnNext.Visible = false;
            ...
            {
                this.lbtnNext.Visible = true;
                this.EditMappingrrTxtEntity = entity;
            }
            ...
            switch (this.Action)
            {
                ...
                case "M":
                    ...
                    for (int i = 1; i < 30; i++)
                    {
                        string strFieldName = "RR_" + i.ToString() + "_S";
                        object ob = entity.GetValue(strFieldName, out rt);
                        if (ob != null)
                        {
                            if (!String.IsNullOrEmpty(ob.ToString()))
        ...
        private bool InitialUI()
        {
            ...
            foreach (Control control in cph.Controls)
            {
                if (control is CheckBox)
                {
                    ...
                    if (ck.ID != "chkS_Id") ck.Checked = false;
                }
            }
            tbxMappingName.Text = "";
            tbxRRCount.Text = "";
                ...
                foreach (Control control in cDiv.Controls)
                {
                    if (control is CheckBox)
                    {
                        ...
                        if (ck.Checked)
                        {
                            ...
                            {
                                case "chkRR":
                                    iCount = Convert.ToInt16(((TextBox)cph.FindControl("tbxRRCount")).Text);
                                    for (int i = 1; i <= iCount; i++)
                                    {
                                        strID = "RR_" + i.ToString();
        ...
        protected void lbtnNext_Click(object sender, EventArgs e)
        {
            ...
            if (ddlFileType.SelectedValue == "xls")  //xls
            {
                ...
                //組 Html
                initStep2B(this.EditMappingrrXlsmdbEntity);
            }
            else
            {
                divStep1.Visible = false;
                ...
                //組 Html
                initStep2A(this.EditMappingrrTxtEntity);
            }
        }

        protected void initStep2B(MappingrrXlsmdbEntity entity)
        {
        ...
        protected void initStep2A(MappingrrTxtEntity entity)
        {
            List<MappingItem> items = this.EditMappingItems;
        ...
        protected void lbtnOK2A_Click(object sender, EventArgs e)
        {
            ...
            if (Action == "A")
            {
                mappingId = GetNewMappingID(MappingrrTxtEntity.TABLE_NAME);
            }
            else
            {
            ...
            itemNames.Add("tableName", MappingrrTxtEntity.TABLE_NAME);
        ...
        protected void lbtnOK2B_Click(object sender, EventArgs e)
        {
            ...
            if (Action == "A")
            {
                mappingId = GetNewMappingID(MappingrrXlsmdbEntity.TABLE_NAME);
            }
            else
            {
                mappingId = this.EditMappingId;
            ...
            itemNames.Add("tableName", MappingrrXlsmdbEntity.TABLE_NAME);
            itemNames.Add("receiveType", receiveType);
            itemNames.Add("mappingId", mappingId);
        ...
        {
            if (tableName == MappingrrTxtEntity.TABLE_NAME)
            {
                MappingrrTxtEntity entity = null;
                Expression where = new Expression(MappingrrTxtEntity.Field.ReceiveType, this.EditReceiveType);
                KeyValueList<OrderByEnum> orderbys = new KeyValueList<OrderByEnum>();
                orderbys.Add(MappingrrTxtEntity.Field.MappingId, OrderByEnum.Desc);

                XmlResult result = DataProxy.Current.SelectFirst<MappingrrTxtEntity>(this, where, orderbys, out entity);
                if (result.IsSuccess)
                {
                    if (entity == null)
                    {
            ...
            {
                MappingrrXlsmdbEntity entity = null;
                Expression where = new Expression(MappingrrXlsmdbEntity.Field.ReceiveType, this.EditReceiveType);
                KeyValueList<OrderByEnum> orderbys = new KeyValueList<OrderByEnum>();
                orderbys.Add(MappingrrXlsmdbEntity.Field.MappingId, OrderByEnum.Desc);

                XmlResult result = DataProxy.Current.SelectFirst<MappingrrXlsmdbEntity>(this, where, orderbys, out entity);
                if (result.IsSuccess)
                {
                    if (entity == null)
                    {
        ...
        public string CheckDataForInputUI()
        {
            ...
            if (chkRR.Checked)
            {
                if (String.IsNullOrEmpty(tbxRRCount.Text))
                {
                    return this.GetLocalized("請輸入") + this.GetLocalized("減免金額項目");
                }
            }
