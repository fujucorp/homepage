...
namespace eSchoolWeb.D
{
    ...
    /// 學分費收費標準
    /// </summary>
    public partial class D1300002 : PagingBasePage
    {
        ...
        protected override XmlResult GetWhereAndOrderBys(out Expression where, out KeyValueList<OrderByEnum> orderbys)
        {
            ...
            where.And(CreditStandardEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(CreditStandardEntity.Field.YearId, this.QueryYearID);
            where.And(CreditStandardEntity.Field.TermId, this.QueryTermID);
            where.And(CreditStandardEntity.Field.DepId, this.QueryDepID);
            where.And(CreditStandardEntity.Field.ReceiveId, this.QueryReceiveID);


            orderbys = new KeyValueList<OrderByEnum>(1);
            orderbys.Add(CreditStandardEntity.Field.CollegeId, OrderByEnum.Asc);
        ...
        protected override XmlResult CallQueryDataAndBind(PagingInfo pagingInfo)
        {
            ...
            XmlResult xmlResult = base.QueryDataAndBind<CreditStandardEntity>(pagingInfo, ucPagings, this.gvResult);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        Server.Transfer(WebHelper.GenRNUrl("D1300002M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        int count = 0;
                        XmlResult result = DataProxy.Current.Delete<CreditStandardEntity>(this, entity, out count);
                        if (result.IsSuccess)
                        {
                            if (count == 0)
                            {
        ...
        /// 取得院別名稱
        /// </summary>
        protected CollegeListEntity[] getAllColleges()
        {
        ...
        /// <returns></returns>
        public static string GetCollegeText(CollegeListEntity[] allColleges, string code, string defaultText)
        {
            if (allColleges!= null && code != null && code.Length > 0)
            {
                foreach (CollegeListEntity data in allColleges)
                {
                    if (data.CollegeId == code)
                    {
        ...
        {
            //院別名稱
            CollegeListEntity[] allCollegeList = getAllColleges();


            CreditStandardEntity[] datas = this.gvResult.DataSource as CreditStandardEntity[];
            ...
            {
                CreditStandardEntity data = datas[row.RowIndex];
                
                //顯示院別名稱
                row.Cells[0].Text = GetCollegeText(allCollegeList, data.CollegeId, string.Empty);


                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4},{5}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReceiveId, data.CollegeId);
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1300002M.aspx");
        }
    }
}