...
{
    public partial class D1300003 : PagingBasePage
    {
        ...
        protected override XmlResult GetWhereAndOrderBys(out Expression where, out KeyValueList<OrderByEnum> orderbys)
        {
            ...
            where.And(CreditbStandardEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(CreditbStandardEntity.Field.YearId, this.QueryYearID);
            where.And(CreditbStandardEntity.Field.TermId, this.QueryTermID);
            where.And(CreditbStandardEntity.Field.DepId, this.QueryDepID);
            where.And(CreditbStandardEntity.Field.ReceiveId, this.QueryReceiveID);


            orderbys = new KeyValueList<OrderByEnum>(1);
            orderbys.Add(CreditbStandardEntity.Field.CreditbId, OrderByEnum.Asc);
        ...
        protected override XmlResult CallQueryDataAndBind(PagingInfo pagingInfo)
        {
            ...
            XmlResult xmlResult = base.QueryDataAndBind<CreditbStandardEntity>(pagingInfo, ucPagings, this.gvResult);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        QueryString.Add("CreditbId", creditbId);
                        Session["QueryString"] = QueryString;

                        #region [MDY:20210521] 原碼修正
                        Server.Transfer(WebHelper.GenRNUrl("D1300003M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        entity.CreditbId = creditbId;
                        #endregion

                        int count = 0;
                        XmlResult result = DataProxy.Current.Delete<CreditbStandardEntity>(this, entity, out count);
                        if (result.IsSuccess)
                        {
                            if (count == 0)
                            {
        ...
        protected void gvResult_PreRender(object sender, EventArgs e)
        {
            ...
            CreditbStandardEntity[] datas = this.gvResult.DataSource as CreditbStandardEntity[];
            ...
            {
                CreditbStandardEntity data = datas[row.RowIndex];
                
                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4},{5}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReceiveId, data.CreditbId);
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1300003M.aspx");
        }
    }
}