...
{
    public partial class D1400004 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetDataAndBind(string qReceiveType)
        {
            ...
            where.And(MappingloXlsmdbEntity.Field.ReceiveType, qReceiveType);
            #endregion
            ...
            MappingloXlsmdbEntity[] XlsDatas = null;
            XmlResult result = DataProxy.Current.SelectAll<MappingloXlsmdbEntity>(this, where, orderbys, out XlsDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
            ...
            MappingloTxtEntity[] TxtDatas = null;
            result = DataProxy.Current.SelectAll<MappingloTxtEntity>(this, where, orderbys, out TxtDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
                this.ShowSystemMessage(this.GetLocalized("查詢資料失敗") + "，" + result.Message);
            ...
            MappingloXlsmdbEntity[] allDatas = new MappingloXlsmdbEntity[iCount];
            for (int i = 0; i < XlsDatas.Length; i++)
            {
                allDatas[i] = new MappingloXlsmdbEntity();
            ...
            {
                allDatas[i + XlsDatas.Length] = new MappingloXlsmdbEntity();
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1400004M.aspx");
        }
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        Server.Transfer(WebHelper.GenRNUrl("D1400004M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        if (args[0] == "xls")
                        {
                            MappingloXlsmdbEntity XlsData = new MappingloXlsmdbEntity();
                            XlsData.ReceiveType = args[1];
                            XlsData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingloXlsmdbEntity>(this, XlsData, out count);
                        }
                        else
                        {
                            MappingloTxtEntity TxtData = new MappingloTxtEntity();
                            TxtData.ReceiveType = args[1];
                            TxtData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingloTxtEntity>(this, TxtData, out count);
                        }
                ...
                case "GenOdsSample":
                    #region 產生試算表範本檔
                    {
                        if (args[0] == "xls")
                        {
                            MappingloXlsmdbEntity data = null;
                            Expression where = new Expression(MappingloXlsmdbEntity.Field.ReceiveType, args[1])
                                .And(MappingloXlsmdbEntity.Field.MappingId, args[2]);
                            XmlResult result = DataProxy.Current.SelectFirst<MappingloXlsmdbEntity>(this, where, null, out data);
                            if (!result.IsSuccess)
                            {
        ...
        {
            MappingloXlsmdbEntity[] datas = this.gvResult.DataSource as MappingloXlsmdbEntity[];
            ...
            {
                MappingloXlsmdbEntity data = datas[row.RowIndex];
