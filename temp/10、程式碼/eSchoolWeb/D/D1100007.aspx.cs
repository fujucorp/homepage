...
namespace eSchoolWeb.D
{
    ...
    /// 減免類別代碼
    /// </summary>
    public partial class D1100007 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetAndBindQueryData()
        {
            ...
            XmlResult xmlResult = DataProxy.Current.SelectAll<ReduceListEntity>(this, where, orderbys, out datas);
            if (!xmlResult.IsSuccess)
            {
        ...
        private Expression GetWhere()
        {
            ...
            {
                where.And(ReduceListEntity.Field.ReceiveType, receiveType);
            }

            string yearId = this.QueryYearID;
            if (!String.IsNullOrEmpty(yearId))
            {
                where.And(ReduceListEntity.Field.YearId, yearId);
            }

            string termId = this.QueryTermID;
            if (!String.IsNullOrEmpty(termId))
            {
                where.And(ReduceListEntity.Field.TermId, termId);
            }

            string DepId = this.QueryDepID;
            if (!String.IsNullOrEmpty(DepId))
            {
                where.And(ReduceListEntity.Field.DepId, DepId);
            }
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1100007M.aspx");
        }

        protected void gvResult_PreRender(object sender, EventArgs e)
        {
            ReduceListEntity[] datas = this.gvResult.DataSource as ReduceListEntity[];
            ...
            {
                ReduceListEntity data = datas[row.RowIndex];
                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReduceId);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        QueryString.Add("ReduceId", args[4]);
                        Session["QueryString"] = QueryString;
