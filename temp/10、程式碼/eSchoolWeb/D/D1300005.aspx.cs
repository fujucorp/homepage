...
{
    public partial class D1300005 : PagingBasePage
    {
        ...
        protected override XmlResult GetWhereAndOrderBys(out Expression where, out KeyValueList<OrderByEnum> orderbys)
        {
            ...
            where.And(LoanStandardEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(LoanStandardEntity.Field.YearId, this.QueryYearID);
            where.And(LoanStandardEntity.Field.TermId, this.QueryTermID);
            where.And(LoanStandardEntity.Field.DepId, this.QueryDepID);
            where.And(LoanStandardEntity.Field.ReceiveId, this.QueryReceiveID);


            orderbys = new KeyValueList<OrderByEnum>(1);
            orderbys.Add(LoanStandardEntity.Field.LoanId, OrderByEnum.Asc);
        ...
        protected override XmlResult CallQueryDataAndBind(PagingInfo pagingInfo)
        {
            ...
            XmlResult xmlResult = base.QueryDataAndBind<LoanStandardEntity>(pagingInfo, ucPagings, this.gvResult);
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        QueryString.Add("LoanId", loanId);
                        Session["QueryString"] = QueryString;

                        #region [MDY:20210521] 原碼修正
                        Server.Transfer(WebHelper.GenRNUrl("D1300005M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        int count = 0;
                        XmlResult result = DataProxy.Current.Delete<LoanStandardEntity>(this, entity, out count);
                        if (result.IsSuccess)
                        {
                            if (count == 0)
                            {
        ...
        /// 取得就貸名稱
        /// </summary>
        protected LoanListEntity[] getAllLoans()
        {
            LoanListEntity[] datas = null;
            Expression where = new Expression();
            where.And(LoanListEntity.Field.ReceiveType, this.QueryReceiveType);
            where.And(LoanListEntity.Field.YearId, this.QueryYearID);
            where.And(LoanListEntity.Field.TermId, this.QueryTermID);
            where.And(LoanListEntity.Field.DepId, this.QueryDepID);
        ...
        /// <returns></returns>
        public static string GetLoanText(LoanListEntity[] allDatas, string code, string defaultText)
        {
            if (allDatas != null && code != null && code.Length > 0)
            {
                foreach (LoanListEntity data in allDatas)
                {
                    if (data.LoanId == code)
                    {
        ...
        {
            //院別名稱
            LoanListEntity[] allData = getAllLoans();


            LoanStandardEntity[] datas = this.gvResult.DataSource as LoanStandardEntity[];
            ...
            {
                LoanStandardEntity data = datas[row.RowIndex];

                //顯示就貸名稱
                row.Cells[1].Text = GetLoanText(allData, data.LoanId, string.Empty);

                //資料參數
                string argument = String.Format("{0},{1},{2},{3},{4},{5}", data.ReceiveType, data.YearId, data.TermId, data.DepId, data.ReceiveId, data.LoanId);
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1300005M.aspx");
        }
    }
}