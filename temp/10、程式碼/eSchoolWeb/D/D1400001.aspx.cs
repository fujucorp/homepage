...
{
    public partial class D1400001 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存商家代號代碼的查詢條件
        ...
        private bool GetDataAndBind(string qReceiveType)
        {
            ...
            where.And(MappingreXlsmdbEntity.Field.ReceiveType, qReceiveType);
            #endregion
            ...
            MappingreXlsmdbEntity[] XlsDatas = null;
            XmlResult result = DataProxy.Current.SelectAll<MappingreXlsmdbEntity>(this, where, orderbys, out XlsDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
            ...
            MappingreTxtEntity[] TxtDatas = null;
            result = DataProxy.Current.SelectAll<MappingreTxtEntity>(this, where, orderbys, out TxtDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
                this.ShowSystemMessage(this.GetLocalized("查詢資料失敗") + "，" + result.Message);
            ...
            MappingreXlsmdbEntity[] allDatas = new MappingreXlsmdbEntity[iCount];
            for (int i = 0; i < XlsDatas.Length; i++)
            {
                allDatas[i] = new MappingreXlsmdbEntity();
            ...
            {
                allDatas[i + XlsDatas.Length] = new MappingreXlsmdbEntity();
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            //判斷是否有 SchoolRid 代收費用檔，如果沒有不可新增
            if (!hasSchoolRid())
            {
                //[TODO] 固定顯示訊息的收集
                string msg = this.GetLocalized("請先建立代收費用檔");
                this.ShowSystemMessage(msg);
                return;
            }
            ...
            Server.Transfer("D1400001M.aspx");
        }

        protected bool hasSchoolRid()
        {
            SchoolRidEntity data = null;
            Expression where = new Expression(SchoolRidEntity.Field.ReceiveType, this.QueryReceiveType);
            XmlResult result = DataProxy.Current.SelectFirst<SchoolRidEntity>(this, where, null, out data);
            if (!result.IsSuccess)
            {
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        Server.Transfer(WebHelper.GenRNUrl("D1400001M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        if (args[0] == "xls")
                        {
                            ...
                            result = DataProxy.Current.Delete<MappingreXlsmdbEntity>(this, XlsData, out count);
                        }
                        else
                        {
                            ...
                            result = DataProxy.Current.Delete<MappingreTxtEntity>(this, TxtData, out count);
                        }
                ...
                case "GenOdsSample":
                    #region 產生試算表範本檔
                    {
                        if (args[0] == "xls")
                        {
                            MappingreXlsmdbEntity data = null;
                            Expression where = new Expression(MappingreXlsmdbEntity.Field.ReceiveType, args[1])
                                .And(MappingreXlsmdbEntity.Field.MappingId, args[2]);
                            XmlResult result = DataProxy.Current.SelectFirst<MappingreXlsmdbEntity>(this, where, null, out data);
                            if (!result.IsSuccess)
                            {
                                this.ShowSystemMessage(result.Code, result.Message);
                                return;
        ...
        {
            MappingreXlsmdbEntity[] datas = this.gvResult.DataSource as MappingreXlsmdbEntity[];
            ...
            {
                MappingreXlsmdbEntity data = datas[row.RowIndex];
