...
{
    public partial class D1400002 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetDataAndBind(string qReceiveType)
        {
            ...
            where.And(MappingrtXlsmdbEntity.Field.ReceiveType, qReceiveType);
            #endregion
            ...
            MappingrtXlsmdbEntity[] XlsDatas = null;
            XmlResult result = DataProxy.Current.SelectAll<MappingrtXlsmdbEntity>(this, where, orderbys, out XlsDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
            ...
            else if (XlsDatas == null || XlsDatas.Length == 0)
            {
            }

            MappingrtTxtEntity[] TxtDatas = null;
            result = DataProxy.Current.SelectAll<MappingrtTxtEntity>(this, where, orderbys, out TxtDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
                this.ShowSystemMessage(this.GetLocalized("查詢資料失敗") + "，" + result.Message);
            ...
            MappingrtXlsmdbEntity[] allDatas = new MappingrtXlsmdbEntity[iCount];
            for (int i = 0; i < XlsDatas.Length; i++)
            {
                allDatas[i] = new MappingrtXlsmdbEntity();
            ...
            {
                allDatas[i + XlsDatas.Length] = new MappingrtXlsmdbEntity();
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1400002M.aspx");
        }
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        Server.Transfer(WebHelper.GenRNUrl("D1400002M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        if (args[0] == "xls")
                        {
                            MappingrtXlsmdbEntity XlsData = new MappingrtXlsmdbEntity();
                            XlsData.ReceiveType = args[1];
                            XlsData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingrtXlsmdbEntity>(this, XlsData, out count);
                        }
                        else
                        {
                            MappingrtTxtEntity TxtData = new MappingrtTxtEntity();
                            TxtData.ReceiveType = args[1];
                            TxtData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingrtTxtEntity>(this, TxtData, out count);
                        }
                ...
                case "GenOdsSample":
                    #region 產生試算表範本檔
                    {
                        if (args[0] == "xls")
                        {
                            MappingrtXlsmdbEntity data = null;
                            Expression where = new Expression(MappingrtXlsmdbEntity.Field.ReceiveType, args[1])
                                .And(MappingrtXlsmdbEntity.Field.MappingId, args[2]);
                            XmlResult result = DataProxy.Current.SelectFirst<MappingrtXlsmdbEntity>(this, where, null, out data);
                            if (!result.IsSuccess)
                            {
        ...
        {
            MappingrtXlsmdbEntity[] datas = this.gvResult.DataSource as MappingrtXlsmdbEntity[];
            ...
            {
                MappingrtXlsmdbEntity data = datas[row.RowIndex];
