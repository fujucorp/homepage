...
namespace eSchoolWeb.C
{
    ...
    /// 查詢問題檔
    /// </summary>
    public partial class C3600001 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存查詢的業務別碼代碼
        ...
        protected void ccbtnExport_Click(object sender, EventArgs e)
        {
            ...
            if (this.GetAndKeepQueryCondition())
            {
                string funcId = "C3600001";
                Expression where = this.GetWhere();
                byte[] fileContent = null;

                #region [MDY:20190906] (2019擴充案) 匯出檔增加 ODS 格式
                XmlResult xmlResult = DataProxy.Current.ExportQueryResult(this.Page, funcId, where, out fileContent, (extName == "ODS"));
                #endregion
        ...
        private bool GetAndKeepQueryCondition()
        {
            ...
            //if (String.IsNullOrEmpty(this.QueryCancelNo)
            //    && (String.IsNullOrEmpty(this.QueryReceiveDateStart) || String.IsNullOrEmpty(this.QueryReceiveDateEnd))
            //    && (String.IsNullOrEmpty(this.QueryAccountDateStart) || String.IsNullOrEmpty(this.QueryAccountDateEnd)))
            //{
            //    this.ShowSystemMessage("虛擬帳號、代收日、入帳日至少需要設定一個條件");
            //    return false;
            //}
            //#endregion
            #endregion
        ...
        protected void gvResult_PreRender(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in this.gvResult.Rows)
            {
                ...
                {
                    row.Cells[8].Text = String.Format("<a href=\"javascript:void(0);\" class=\"btn\" onclick=\"openDialog('{0}','')\">編輯</a>", row.RowIndex);
                }
                else
                {
                    row.Cells[8].Text = String.Format("<a href=\"javascript:void(0);\" class=\"btn\" onclick=\"openDialog('{0}','{1}')\">編輯</a><br/>{1}", row.RowIndex, data.ProblemRemark);
                }
            }
        }
        ...
        protected void ccbtnOK_Click(object sender, EventArgs e)
        {
            ...
            int rowIndex = 0;
            if (!Int32.TryParse(this.hidRowIndex.Value.Trim(), out rowIndex) || rowIndex < 0 || rowIndex >= this.gvResult.Rows.Count)
            {
                //[TODO] 固定顯示訊息的收集
            ...
            string txtProblemId = this.gvResult.Rows[rowIndex].Cells[0].Text.Trim();
            int problemId = 0;
            if (!Int32.TryParse(txtProblemId, out problemId) || problemId < 1)
            {
                //[TODO] 固定顯示訊息的收集
                string msg = this.GetLocalized("無法取得要處理資料的參數!");
                this.ShowSystemMessage(msg);
            ...
            string problemRemark = this.hidProblemRemark.Value.Trim();

            Expression where = new Expression(ProblemListEntity.Field.Id, problemId);
            KeyValue[] fieldValues = new KeyValue[] {
                new KeyValue(ProblemListEntity.Field.ProblemRemark, problemRemark)
            };

            string action = this.GetLocalized("編輯備註");
            int count = 0;
            XmlResult xmlResult = DataProxy.Current.UpdateFields<ProblemListEntity>(this.Page, where, fieldValues, out count);
            if (xmlResult.IsSuccess)
            {
                if (count == 0)
                {
