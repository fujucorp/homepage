<%@ Page Title="土地銀行 - 代收學雜費服務網 - 產生銷帳資料檔(跨學年)" Language="C#" MasterPageFile="~/MasterPage/Main.Master" AutoEventWireup="true" CodeBehind="C3700008.aspx.cs" Inherits="eSchoolWeb.C.C3700008" %>
...
<uc:Filter1 ID="ucFilter1" runat="server" UIMode="Option" ReceiveTypeReadonly="true" YearDefaultKind="All" YearDefaultMode="ByKind" TermDefaultKind="All" TermDefaultMode="ByKind" OnItemSelectedIndexChanged="ucFilter1_ItemSelectedIndexChanged" Filter2ControlID="ucFilter2" />
<uc:Filter2 ID="ucFilter2" runat="server" ReceiveDefaultKind="All" ReceiveDefaultMode="ByKind" UIMode="Option" OnItemSelectedIndexChanged="ucFilter2_ItemSelectedIndexChanged" />
...
<tr>
    <td colspan="2">
        <div align="left">
            <span><asp:RadioButton ID="rbtNoQueryDate" runat="server" GroupName="rbtDateType" Checked="true" /><%=this.GetLocalized("不指定日期區間") %></span>
            <span><asp:RadioButton ID="rbtReceiveDate" runat="server" GroupName="rbtDateType" /><%=this.GetLocalized("代收日區間") %></span>
            <span><asp:RadioButton ID="rbtAccountDate" runat="server" GroupName="rbtDateType" /><%=this.GetLocalized("入帳日區間") %></span>
...
<table class="condition" summary="匯出資料項目" width="100%">
<tr>
	<th colspan="4"><div align="left"><%= GetLocalized("勾選匯出資料項目") %></div></th>
</tr>
<tr>
	<td colspan="4">
		載入資料項目設定：
		...
		<asp:LinkButton ID="lbtnLoadExportConfig" runat="server" CssClass="btn" OnClick="lbtnLoadExportConfig_Click">載入</asp:LinkButton>
	</td>
</tr>
<tr>
...
<tr>
	<td id="tdSaveExportConfig" style="padding: 2px 0px;">
		<div style="margin-bottom:5px;">存入資料項目設定：
			<asp:DropDownList ID="ddlSaveExportConfig" runat="server">
			</asp:DropDownList>
		</div>
		<div style="margin-top:5px;">名稱：
			<asp:TextBox ID="tbxSaveExportConfigName" runat="server" MaxLength="50" Width="80%"></asp:TextBox>&nbsp;&nbsp;
			<asp:LinkButton ID="lbtnSaveExportConfig" runat="server" CssClass="btn" OnClick="lbtnSaveExportConfig_Click">儲存</asp:LinkButton>
		</div>
	</td>
</tr>
</table>
...
<div class="button">
    <cc:MyLinkButton ID="ccbtnGenFile" runat="server" LocationText="產生檔案XLS" CommandArgument="XLS" OnClick="ccbtnGenFile_Click"></cc:MyLinkButton>
    <cc:MyLinkButton ID="ccbtnGenFileODS" runat="server" LocationText="產生檔案ODS" CommandArgument="ODS" OnClick="ccbtnGenFile_Click"></cc:MyLinkButton>
    <cc:MyGoBackButton ID="ccbtnGoBack" runat="server"></cc:MyGoBackButton>
</div>

<div >
	<div style="margin-bottom:10px;">檔案清單 (最近 90 天的最新 30 筆資料)&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		<cc:MyLinkButton ID="ccbtnReloadFileList" runat="server" LocationText="更新清單" CssClass="btn" OnClick="ccbtnReloadFileList_Click"></cc:MyLinkButton>
	</div>
	<asp:GridView ID="gvResult" runat="server" CssClass="modify"
		...
		<Columns>
			<cc:MyBoundField DataField="CrtDate" LocationHeaderText="建立日期" DataFormatString="{0:yyyy/MM/dd HH:mm:ss}">
				<HeaderStyle Wrap="False"></HeaderStyle>
			</cc:MyBoundField>
			<cc:MyBoundField DataField="Explain" LocationHeaderText="說明">
				<HeaderStyle Wrap="True"></HeaderStyle>
			</cc:MyBoundField>
			<cc:MyBoundField DataField="StatusText" LocationHeaderText="狀態">
			...
			<cc:MyTemplateField LocationHeaderText="下載">
				...
				<ItemTemplate>
					<asp:LinkButton ID="lbtnDownload" runat="server" CssClass="btn">下載</asp:LinkButton>
				</ItemTemplate>
			</cc:MyTemplateField>
		</Columns>
	</asp:GridView>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#tdSaveExportConfig select').change(function () {
            var name = $('#tdSaveExportConfig input[type=text]').val().trim();
            if (name.length == 0) {
