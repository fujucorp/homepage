...
UPDATE [{0}] SET [{1}] = @FileName, [{2}] = @ExtName, [{3}] = @ProcessStatus, [{4}] = GETDATE(), [{5}] = 'SYSTEM' 
 WHERE [{3}] = @WaitStatus AND [{6}] = @SN AND [{7}] = @ReceiveType AND [{8}] = @Kind ;"
                ...
                try
                {
                    #region 取得各費用別的繳款期限、超商延遲日與要採用的收入科目數量名稱、備註標題
...
SELECT [Receive_Type], [Year_Id], [Term_Id], [Dep_Id], [Receive_Id], ISNULL([Pay_Date], '') AS [Pay_Date], ISNULL([Extra_Days], 0) AS [Extra_Days]
 ...
 WHERE " + String.Join(" AND ", whereSqls);
                        ...
                        List<string> tmpItemNames = new List<string>(40);   //暫存收入科目名稱
                        foreach (DataRow drow in dt.Rows)
                        {
                            #region 繳款期限、超商延遲日
                            {
                            ...
                            for (int no = 1; no <= 40; no++)
                            {
                                string fieldName = String.Format("Receive_Item{0:00}", no);

                                #region [MDY:20170708] Row[] 取值後 Replace 單引號 (For Heuristic SQL Injection 修改)
                                string fieldValue = drow[fieldName].ToString().Replace("'", "").Replace("[", "").Replace("]", "");
                                #endregion
                                ...
                                {
                                    tmpItemNames.Add(fieldValue);
                                }
                            }
                            #endregion

                            //如果暫存收入科目名稱數量比採用的收入科目名稱數量多，改採用暫存收入科目名稱
                            if (receiveItemNames.Count < tmpItemNames.Count)
                            {
                                receiveItemNames.Clear();
                                receiveItemNames.AddRange(tmpItemNames);
...
//        }
 ...
 WHERE Receive_Type = @ReceiveType AND Year_Id = @YearId AND Term_Id = @TermId AND Dep_Id = @DepId AND Receive_Id = @ReceiveId");
                    ...
                    for (int no = 1; no <= 40; no++)
                    {
                        ...
                        if (!String.IsNullOrEmpty(name))
                        {
                            receiveItems.Add(String.Format("Receive_{0:00}", no), name.Replace("'", ""));
                        }
                    }
 ...
 WHERE RL.Receive_Type = @ReceiveType AND RL.Year_Id = @YearId AND RL.Term_Id = @TermId AND RL.Dep_Id = @DepId AND RL.Receive_Id = @ReceiveId";
                ...
                if (reportKind == "1")
                {
                    ...
                    List<DataTable> dtDatas = new List<DataTable>(receiveItems.Count);
                    foreach (KeyValue<string> receiveItem in receiveItems)
                    {
                        DataTable dtData = null;
                        string receiveItemKey = receiveItem.Key;
                        string receiveItemName = receiveItem.Value;
