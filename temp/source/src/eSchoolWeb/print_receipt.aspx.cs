...
{
    public partial class print_receipt : LocalizedPage  //System.Web.UI.Page, IMenuPage
    {
        #region Const
        /// <summary>
        ...
        private bool getStudentReceive(string cancel_no, string stuId, out StudentReceiveView3[] student_receives, out string msg)
        {
            ...
            #region [MDY:20210301] 改為商家代號 + 虛擬帳號 + 已繳條件 (View 已做 NULL 轉空字串 處理)
            Expression where = new Expression(StudentReceiveView3.Field.ReceiveType, cancel_no.Substring(0, 4))
                ...
                .And(StudentReceiveView3.Field.ReceiveWay,RelationEnum.NotEqual, String.Empty);
            #endregion

            #region [Old] (不判斷了) 開放列印條件 (View 已做 NULL 轉空字串 處理)
        ...
        private bool GenReceipt()
        {
            ...
            if (xmlResult.IsSuccess)
            {
                #region [Old] 土銀不分收據, 憑單
                //string pdfName = (this.EditCancelStatus == CancelStatusCodeTexts.CANCELED) ? "收據" : "憑單";
                //string fileName = String.Format("{0}繳費{1}.PDF", EditStuId, pdfName);
                #endregion

                #region [MDY:20210401] 原碼修正
                string fileName = String.Format("{0}繳費收據.PDF", HttpUtility.UrlEncode(EditStuId));
                #endregion
            ...
            else
            {
                ...
                //js.AppendFormat("alert('{0}');", HttpUtility.JavaScriptStringEncode("產生收據發生錯誤，請稍後再試")).AppendLine();
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                UCPageNews.PageId = BoardTypeCodeTexts.PRINTRECEIVE;
            }
        }
