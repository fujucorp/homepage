    ...
    public partial class D1200001M : BasePage
    {
        ...
        private bool CheckEditData(SchoolRidEntity data)
        {
            ...
            if (isEngEnabled)
            {
                ...
                #region 收據英文模板
                if (!String.IsNullOrEmpty(data.InvoiceformId) && String.IsNullOrEmpty(data.InvoiceFormEId))
                {
                    this.ShowSystemMessage("指定收據的中文模板就必須指定其英文模板");
                    return false;
                }
                #endregion
                ...
                #region 收入科目英文名稱
                {
                    string[] chtReceiveItems = data.GetAllReceiveItemChts();
                    string[] engReceiveItems = data.GetAllReceiveItemEngs();
                    for (int idx = 0; idx < chtReceiveItems.Length; idx++)
                    {
                        if (!String.IsNullOrEmpty(chtReceiveItems[idx]) && String.IsNullOrEmpty(engReceiveItems[idx]))
                        {
                            this.ShowSystemMessage($"指定收入科目{idx + 1:00}的中文名稱就必須指定其英文名稱");
                            return false;
                        }
                    }
                }
                ...
                #region 備註項目英文標題
                {
                    string[] chtMemoTitles = data.GetAllMemoTitleChts();
                    string[] engMemoTitles = data.GetAllMemoTitleEngs();
                    for (int idx = 0; idx < chtMemoTitles.Length; idx++)
                    {
                        if (!String.IsNullOrEmpty(chtMemoTitles[idx]) && String.IsNullOrEmpty(engMemoTitles[idx]))
                        {
                            this.ShowSystemMessage($"指定備註項目{idx + 1:00}的中文標題就必須指定其英文標題");
                            return false;
                        }
                    }
                }
                ...
                #region 注意事項英文內容
                {
                    string[] chtBriefs = data.GetAllBriefChts();
                    string[] engBriefs = data.GetAllBriefEngs();
                    for (int idx = 0; idx < chtBriefs.Length; idx++)
                    {
                        if (!String.IsNullOrEmpty(chtBriefs[idx]) && String.IsNullOrEmpty(engBriefs[idx]))
                        {
                            this.ShowSystemMessage($"指定注意事項{idx + 1:00}的中文內容就必須指定其英文內容");
                            return false;
                        }
                    }
                }
