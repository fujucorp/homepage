...
{
    public partial class D1400003 : BasePage
    {
        #region Property
        /// <summary>
        /// 儲存業務別碼代碼的查詢條件
        ...
        private bool GetDataAndBind(string qReceiveType)
        {
            ...
            where.And(MappingcsXlsmdbEntity.Field.ReceiveType, qReceiveType);
            #endregion
            ...
            MappingcsXlsmdbEntity[] XlsDatas = null;
            XmlResult result = DataProxy.Current.SelectAll<MappingcsXlsmdbEntity>(this, where, orderbys, out XlsDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
            ...
            MappingcsTxtEntity[] TxtDatas = null;
            result = DataProxy.Current.SelectAll<MappingcsTxtEntity>(this, where, orderbys, out TxtDatas);
            if (!result.IsSuccess)
            {
                //[TODO] 固定顯示訊息的收集
                this.ShowSystemMessage(this.GetLocalized("查詢資料失敗") + "，" + result.Message);
            ...
            MappingcsXlsmdbEntity[] allDatas = new MappingcsXlsmdbEntity[iCount];
            for (int i = 0; i < XlsDatas.Length; i++)
            {
                allDatas[i] = new MappingcsXlsmdbEntity();
            ...
            {
                allDatas[i + XlsDatas.Length] = new MappingcsXlsmdbEntity();
        ...
        protected void ccbtnInsert_Click(object sender, EventArgs e)
        {
            ...
            Server.Transfer("D1400003M.aspx");
        }
            ...
            switch (e.CommandName)
            {
                case ButtonCommandName.Modify:
                    #region 修改資料
                    {
                        ...
                        Server.Transfer(WebHelper.GenRNUrl("D1400003M.aspx"));
                        #endregion
                    }
                    #endregion
                    break;
                case ButtonCommandName.Delete:
                    #region 刪除資料
                    {
                        ...
                        if (args[0] == "xls")
                        {
                            MappingcsXlsmdbEntity XlsData = new MappingcsXlsmdbEntity();
                            XlsData.ReceiveType = args[1];
                            XlsData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingcsXlsmdbEntity>(this, XlsData, out count);
                        }
                        else
                        {
                            MappingcsTxtEntity TxtData = new MappingcsTxtEntity();
                            TxtData.ReceiveType = args[1];
                            TxtData.MappingId = args[2];
                            result = DataProxy.Current.Delete<MappingcsTxtEntity>(this, TxtData, out count);
                        }
                ...
                case "GenOdsSample":
                    #region 產生試算表範本檔
                    {
                        if (args[0] == "xls")
                        {
                            MappingcsXlsmdbEntity data = null;
                            Expression where = new Expression(MappingcsXlsmdbEntity.Field.ReceiveType, args[1])
                                .And(MappingcsXlsmdbEntity.Field.MappingId, args[2]);
                            XmlResult result = DataProxy.Current.SelectFirst<MappingcsXlsmdbEntity>(this, where, null, out data);
                            if (!result.IsSuccess)
                            {
        ...
        {
            MappingcsXlsmdbEntity[] datas = this.gvResult.DataSource as MappingcsXlsmdbEntity[];
            ...
            {
                MappingcsXlsmdbEntity data = datas[row.RowIndex];
