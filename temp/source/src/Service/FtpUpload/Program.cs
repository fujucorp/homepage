                        ...
                        if (!String.IsNullOrWhiteSpace(argTxt))
                        {
                                ...
                                if (kvs.Length == 2)
                                {
                                    ...
                                    switch (key)
                                    {
                                        ...
                                        case "local_file":
                                            ...
                                            if (!String.IsNullOrEmpty(value))
                                            {
                                                #region [MDY:20220910] Checkmarx - Stored Path Traversal 修正
                                                localFile = value;
                                                if (localFile.IndexOf(@"/") > -1 || localFile.IndexOf(@"\") > -1
                                                    || localFile.IndexOf("..") > -1 || localFile.IndexOf("'") > -1)
                                                {
                                                    isOK = false;
                                                    log.AppendLine("local_file 參數值不正確 (不可包含路徑)");
                                                }
                                                //[MEMO] Checkmarx 一定要 Replace 這四個字元 只好加這行
                                                localFile = localFile.Replace(@"/", "").Replace(@"\", "").Replace("..", "").Replace("'", "");
                                                #endregion
                                            }
                                            break;
                                            #endregion
                            ...
                            if (isOK)
                            {
                                if (protocol == "FTP" || protocol == "FTPS")
                                {
                                    ...
                                    hostConfig = HostConfig.CreateByFTPx(host, uid, pxx, port, useSSL, usePassive, encoding: encoding, workingDirectory: remotePath);
                                    #endregion
                                }
                                else if (protocol == "SFTP")
                                {
                                    hostConfig = HostConfig.CreateBySFTP(host, uid, pxx, port, encoding: encoding, workingDirectory: remotePath);
                                }
                                else
                                {
                                    isOK = false;
                ...
                if (exitCode == 0)
                {
                    ...
                    if (String.IsNullOrEmpty(errmsg))
                    {
                        ...
                        if (String.IsNullOrEmpty(errmsg))
                        {
                            try
                            {
                                #region [MDY:20220723] Checkmarx 調整 (Stored Path Traversal)
                                workPath = Path.GetFileName(workPath);
                                if (!workPath.StartsWith(@"D:\WEB\DATA\CTCB\", StringComparison.InvariantCultureIgnoreCase)
                                    && workPath.StartsWith(@"D:\WEB\DATA\CANCELED\", StringComparison.InvariantCultureIgnoreCase))
                                {
                                    errmsg = $"指定工作路徑 {workPath} 不合法";
                                }
                        ...
                        if (String.IsNullOrEmpty(errmsg))
                        {
                            ...
                            {
                                #region [MDY:20220723] Checkmarx 調整 (Stored Path Traversal)
                                bakPath = Path.GetFileName(bakPath);
                                if (!bakPath.StartsWith(@"D:\WEB\DATA\CTCB\", StringComparison.InvariantCultureIgnoreCase)
                                    && bakPath.StartsWith(@"D:\WEB\DATA\CANCELED\", StringComparison.InvariantCultureIgnoreCase))
                                {
                                    errmsg = $"指定工作路徑 {bakPath} 不合法";
                                }
