...
namespace GenTaxData
{
    class Program
    {
        ...
        private class FileLoger
        {
            ...
            public void FillLog(string msg)
            {
                if (!String.IsNullOrEmpty(_LogFileName) && msg != null)
                {
                    try
                    {
                        ...
                        //else
                        //{
                        //    File.AppendAllText(_LogFileName, $"[{DateTime.Now:HH:mm:ss}] {msg}");
                        //}
                        //#endregion
                        #endregion
            ...
            public void FillLog(string format, params object[] args)
            {
                if (!String.IsNullOrEmpty(format) && args != null && args.Length > 0)
                {
                    try
                    {
                        this.FillLog(String.Format(format, args));
                    }
                    catch (Exception)
                    {
                    }
            ...
            public void FillDebugLog(string msg)
            {
                if (this.IsDebug)
                {
                    this.FillLog(msg);
            ...
            public void FillDebugLog(string format, params object[] args)
            {
                if (this.IsDebug)
                {
                    this.FillLog(format, args);
                }
            }
            #endregion
        }
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                if(!correctID)
                                {
                                    ...
                                    //[MEMO] checkmarx 誤判 log 是路徑的一部分，所以不紀錄 row 了
                                    log = $"第 {recordNo} 筆資料的身分證字號有誤";
                                    #endregion
                                ...
                                if(correctID && correctLevel)
                                {
                                    if(edu_amount==0 && stay_amount==0)
                                    {
                                        ...
                                        //[MEMO] checkmarx 誤判 log 是路徑的一部分，所以不紀錄 row 了
                                        log = $"第 {recordNo} 筆資料的申報金額為0，無需申報";
                                        #endregion
            ...
            try
            {
                ...
                JobcubeEntity job = null;
                {
                    ...
                    if (result.IsSuccess)
                    {
                        ...
                        {
                            exitCode = -1;
                            fileLog.FillDebugLog("查無待處理的 {0} 批次處理佇列資料", jobcubeType);
