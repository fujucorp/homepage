...
namespace FolderClear
{
    ...
    public class Setting
    {
        ...
        /// 正規表示式 前置字串
        /// </summary>
        public static readonly string RegexPrefix = "[REGEX:";
        /// <summary>
        /// 正規表示式 結束字串
        /// </summary>
        public static readonly string RegexSuffix = "]";

        /// <summary>
        /// 最小保留檔案天數 (1)
        /// </summary>
        public static readonly int MinKeepDays = 1;

        /// <summary>
        /// 最大保留檔案天數 (3650)
        /// </summary>
        public static readonly int MaxKeepDays = 3650;

        /// <summary>
        /// Xml 編碼
        /// </summary>
        private static readonly Encoding _XmlEncoding = Encoding.UTF8;  //new UTF8Encoding(false);

        /// <summary>
        /// Unicode 檔案的前導字元
        /// </summary>
        private static readonly char Char65279 = (char)65279;	//'\uFEFF';	//'\xFEFF'; //(char)0xFEFF
        #endregion
        ...
        #region
        /// <summary>
        /// 處理 sub_folder_filter 的正規表示式
        /// </summary>
        public string GenSubFolderRegex()
        {
            if (!String.IsNullOrEmpty(this.SubFolderFilter) && this.SubFolderFilter.StartsWith(RegexPrefix) && this.SubFolderFilter.EndsWith(RegexSuffix))
            {
                try
                {
                    string pattern = this.SubFolderFilter.Substring(RegexPrefix.Length, this.SubFolderFilter.Length - RegexPrefix.Length - 1).Trim();
                    if (pattern.Length > 0)
                    {
                        this.SubFolderRegex = new Regex(pattern, RegexOptions.Compiled);
                    }
                    else
                    {
                        return "缺少正規表示式 pattern";
                    }
                }
                catch
                {
                    return "正規表示式 pattern 不正確";
                }
            }
            return null;
        }
        ...
        public static List<Setting> LoadBySettingFile(out string logPath, out string errmsg)
        {
            ...
            try
            {
                ...
                {
                    string xml = File.ReadAllText(fileName, _XmlEncoding);
                    XmlDocument xDoc = new XmlDocument();
                    if (xml[0] == Char65279)
                    {
                        xDoc.LoadXml(xml.Substring(1));
                    }
                    else
                    {
                        xDoc.LoadXml(xml);
                                ...
                                if (String.IsNullOrEmpty(errmsg))
                                {
                                    ...
                                    if (xMyNode != null)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            #region [OLD]
                                            //if (!String.IsNullOrEmpty(setting.SubFolderFilter) && setting.SubFolderFilter.StartsWith(_RegexPrefix) && setting.SubFolderFilter.EndsWith(_RegexSuffix))
                                            //{
                                            //    try
                                            //    {
                                            //        string pattern = setting.SubFolderFilter.Substring(_RegexPrefix.Length, setting.SubFolderFilter.Length - _RegexPrefix.Length - 1).Trim();
                                            //        if (pattern.Length > 0)
                                            //        {
                                            ...
                                            string errmsg2 = setting.GenSubFolderRegex();
                                            if (!String.IsNullOrEmpty(errmsg2))
                                            {
                                                errmsg = String.Format("第{0}個設定節點的 sub_folder_filter 設定值不正確 ({1})", no, errmsg2);
                                            }
                                            #endregion
                                        }
                                    }
                ...
                {
                    errmsg = String.Format("找不到設定檔 ({0}) 或設定檔無資料", fileName);
                }
            }
            catch (Exception ex)
            {
        ...
        public static string WriteToSettingFile(string logPath, List<Setting> settingDatas)
        {
            string errmsg = null;
            try
            {
                ...
                using (XmlWriter xw = XmlWriter.Create(ms, settings))
                {
                    ...
                    #region logPath
                    xw.WriteElementString("logPath", logPath);
                    #endregion

                    #region setting
                    if (settingDatas != null && settingDatas.Count > 0)
                    {
                        foreach (Setting settingData in settingDatas)
                        {
                            xw.WriteStartElement("setting");

                            xw.WriteElementString("folder_path", settingData.FolderPath);
                            xw.WriteElementString("file_filter", settingData.FileFilter);
                            xw.WriteElementString("sub_folder_filter", settingData.SubFolderFilter);
                            xw.WriteElementString("sub_file_filter", settingData.SubFileFilter);
                            xw.WriteElementString("keep_days", settingData.KeepDays.ToString());
                    ...
                    xw.Flush();
                    xml = _XmlEncoding.GetString(ms.ToArray());
                    if (xml.Length > 0 && xml[0] == Char65279)
                    {
                        xml = xml.Substring(1);
                    }
                }
                ...
                #region [MDY:20220910] Checkmarx - Stored Path Traversal 誤判調整
                #region [OLD] 20220907 沒過
                //#region [MDY:20220905] Checkmarx - Stored Path Traversal 誤判調整
                ////string filePath = System.AppDomain.CurrentDomain.BaseDirectory;
                ////string fileName = Path.Combine(filePath, "Setting.xml");

                ////[MEMO] Checkmarx 一定要 Replace 這四個字元 只好加這行
                //string fileName = "Setting.xml".Replace(@"/", "").Replace(@"\", "").Replace("..", "").Replace("'", "");
                //#endregion

                //File.WriteAllText(fileName, xml, _XmlEncoding);
                #endregion

                string fileName = Path.Combine(System.AppDomain.CurrentDomain.BaseDirectory, "Setting.xml");
                using (var sw = File.Create(fileName))
                {
                    Byte[] buff = _XmlEncoding.GetBytes(xml);
